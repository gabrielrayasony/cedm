...
class BaseDiffEq(abc.ABC):
    ...
    @property
    def condition_labels(self) -> Union[Tensor, None]:
...
class BaseDiffEqSolver(abc.ABC):
    ...
    def solve(
        ...
        ode: BaseDiffEq,
        euler_last_step: Optional[bool] = None,
    ) -> Tensor:
...
class KarrasStochasticDiffEqSolver(BaseDiffEqSolver):
    ...
    def solve(
        self, x: Tensor, y0_tuple: tuple, ode: BaseDiffEq, euler_last_step: Optional[bool] = None
    ) -> Tensor:
        if not len(y0_tuple) == 1:
            raise ValueError(
...
def sample_trajectory_batch(
    ...
    batch_size: int,
    n_steps: int,
    condition_label: Optional[int] = None,
