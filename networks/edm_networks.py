    ...
    def forward(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        ...
        if self.label_dim > 0:
            if class_labels is None:
                class_labels = torch.zeros([x.shape[0], self.label_dim], device=x.device, dtype=torch.float32)
            else:
                class_labels = class_labels.to(torch.float32)
                if class_labels.dim() == 1:  # If labels are not one-hot encoded
                    class_labels = F.one_hot(class_labels.long(), num_classes=self.label_dim).float()
...
#     """MLP architecture for 2D data."""
#     def __init__(
#         self,
#         input_dim,                    # Number of input channels (2 for 2D data)
#         out_channels,                   # Number of output channels (2 for 2D data)
#         label_dim       = 0,            # Number of class labels, 0 = unconditional
#         hidden_dim      = 256,          # Hidden dimension of the MLP
#         num_layers      = 4,            # Number of MLP layers
#         dropout         = 0.1,          # Dropout probability
#         embedding_dim   = 8,          # Dimension of time and label embeddings
#     ):
#         super().__init__()
...
#         # Time embedding
#         self.time_embed = PositionalEmbedding(embedding_dim)
        
#         # Label embedding if needed
#         if label_dim > 0:
#             self.label_embed = Linear(label_dim, embedding_dim)
            
#         # Main MLP layers
#         layers = []
#         in_dim = input_dim + embedding_dim + (embedding_dim if label_dim > 0 else 0)
        
#         for i in range(num_layers):
#             out_dim = hidden_dim if i < num_layers - 1 else out_channels
#             layers.extend([
#                 Linear(in_dim, out_dim),
#                 torch.nn.SiLU() if i < num_layers - 1 else torch.nn.Identity(),
#                 torch.nn.Dropout(dropout) if i < num_layers - 1 else torch.nn.Identity()
#             ])
#             in_dim = out_dim
...
#         # Label embedding if provided
#         if y is not None and self.label_dim > 0:
...
#         # Forward through MLP
#         return self.mlp(x)
from networks.networks_2d import MLP
...
class MLPPrecond(nn.Module):
    def __init__(
        ...
        time_dim: int = 8,
        sigma_data: float = 0.5,    # Add sigma_data
    ):
        super().__init__()
        self.in_channels = in_channels
        self.label_dim = label_dim
    ...
    def forward(self, x: torch.Tensor, sigma: torch.Tensor, y: Optional[torch.Tensor] = None) -> torch.Tensor:
        # Make sure sigma has proper shape
        sigma = sigma.to(x.dtype).reshape(-1, 1)
        ...
        F_x = self.model(x_in, c_noise.flatten())
